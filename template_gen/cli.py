...
def _load_presets_interactive() -> Optional[dict]:
    ...
    choices = []
    for p in presets:
        label = f"{p['name']} ({p['template_name']})"
...
def new(template: Optional[str], output: Optional[str], preset: Optional[str],
    ...
    click.secho(f"\n  Template: {config.name}", fg="cyan", bold=True)
    ...
    if not preset_vars and not no_interactive:
        preset_data = _load_presets_interactive()
        if preset_data:
            preset_vars = preset_data.get("variables", {})
    ...
    if not dry_run and not no_interactive:
        ...
        if save:
            ...
            path = save_preset(preset_name, config.name, template_vars)
...
def preset_save(name: str):
    ...
    path = save_preset(name, config.name, variables)
    click.secho(f"Preset saved: {path}", fg="green")
...
def preset_list():
    ...
    for p in presets:
        click.echo(f"  • {p['name']} (template: {p['template_name']})")
...
def preset_show(name: str):
    ...
    click.secho(f"Template: {data['template_name']}", fg="cyan")
...
def update(project_dir: str, template: str, no_backup: bool, no_interactive: bool, dry_run: bool):
    ...
    click.secho(f"\n  Updating project: {project_dir}", fg="cyan")
    click.secho(f"  Template: {config.name}", fg="cyan")
    ...
    if no_interactive:
        ...
        preset_data = _load_presets_interactive()
        preset_vars = preset_data.get("variables", {}) if preset_data else None
        template_vars = collect_variables(config, presets=preset_vars)
